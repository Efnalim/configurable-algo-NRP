...
with Diagram("Realtionships between conditions", show=False):
    with Cluster("Hard constraints"):
        h13 = C4Node("HC13", "", "Planned vacation", "constraint")
        h12 = C4Node("HC12", "", "Maximum shifts of specific type (night)", "constraint")
    ...
    with Cluster("Soft constraints"):
        s8 = C4Node("SC8", "", "Overtime preferences", "constraint")
        s7 = C4Node("SC7", "", "Missing required skill - exception", "constraint")
    ...
    h8 >> Edge(label="affects") >> s8
    s6 >> Edge(label="affects") << s8
    h13 >> Edge(label="possibly contradicts") << h8
